...
namespace SysLog.Repository
{
	...
	public class SysGlobalExceptionLogRepository : Repository<SysGlobalExceptionLog>, ISysGlobalExceptionLogRepository
	{
        ...
        /// 查询可以删除的列表
        /// </summary>
        /// <param name="createTime">结束时间</param>
        ///  <returns>分页</returns>
        public async Task<IEnumerable<SysGlobalExceptionLog>> GetListDeletableAsync(DateTime createTime)
        {
            return await DbSet
                .AsNoTracking()
                .Where(w => w.CreateTime < createTime)
                .Select(x => new SysGlobalExceptionLog() { Id = x.Id, CreateTime = x.CreateTime })
                .ToListAsync();
        }
    }
}
