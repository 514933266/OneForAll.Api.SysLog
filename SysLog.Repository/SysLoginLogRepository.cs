...
namespace SysLog.Repository
{
	...
	public class SysLoginLogRepository : Repository<SysLoginLog>, ISysLoginLogRepository
	{
		...
		public async Task<IEnumerable<SysLoginLog>> GetListTodayAsync(Guid tenantId, int top)
		{
			var predicate = PredicateBuilder.Create<SysLoginLog>(w => w.SysTenantId == tenantId && w.CreateTime >= DateTime.Now.Date);
