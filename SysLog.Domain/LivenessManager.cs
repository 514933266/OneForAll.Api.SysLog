...
namespace SysLog.Domain
{
    ...
    /// 活跃度
    /// </summary>
    public class LivenessManager : BaseManager, ILivenessManager
    {
        ...
        public LivenessManager(
            ...
            ISysApiLogRepository repository) : base(httpContextAccessor)
        {
            _mapper = mapper;
		...
		public async Task<UserLivenessAggr> GetAsync(DateTime startTime, DateTime endTime)
        {
            ...
            var result = new UserLivenessAggr() { UserId = LoginUser.Id.TryGuid(), StartDate = startTime, EndDate = endTime };
            try
            {
                ...
                {
                    // 没有缓存，直接读库
                    var data = await _repository.GetListAsync(LoginUser.TenantId.ToString(), startTime, endTime);
